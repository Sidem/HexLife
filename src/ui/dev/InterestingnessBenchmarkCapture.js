...
const STAGE0_POSITIVE_HEXES = new Set([
    '004180808001D20490122500400D0800', '400811081000190281004A0844200020',
    '0860021D400412C2C8008E110205A182', '0088800010028810820000F431000D08',
    '00248110000200838101085400000010', '2505020A152DFF2CBB21326B0277901F',
    '120C11B442568E21134E30A85A40C880', '12482080480080006880800180010117',
    '8157327A1F4C7AC80152275D1C3F67B7', '84304E4024A82000162D5CB263E49A49',
    '12080081400181171249219649169669', '12482080480080016880811580170737',
    '16686880688080000117177E177E7EE8', '16686881688181160117177E177E7EE8',
    '6880800180010116E880800080000001', '16686880688080006880800080000001',
]);
...
function normalizeMetrics(result, icLabel = null) {
    ...
    if (icLabel != null) rest.icLabel = icLabel;
    return rest;
}
...
async function capturePanel(appContext, library, negatives, onProgress) {
    ...
    for (let offset = 0; offset < panel.length; offset += worldCount) {
        ...
        const metrics = await Promise.all(batch.map((entry, index) =>
            captureBurst(appContext.worldManager, entry, SCREEN_TICKS, index)
        ));
        onProgress(`Confirming ${offset + 1}–${offset + batch.length} of ${panel.length}…`);
        const confirmMetrics = await Promise.all(batch.map((entry, index) =>
            captureBurst(appContext.worldManager, entry, CONFIRM_TICKS, index)
        ));
        batch.forEach((entry, index) => entries.push({
            ...entry,
            metrics: metrics[index],
            confirmMetrics: confirmMetrics[index],
        }));
    }
    const positives = entries.filter((entry) => entry.label === 'interesting');
    return {
        _meta: {
            description: 'Complete public-library statistical interestingness benchmark. Generated by ?headless=1&benchmark=1; never hand-edit.',
            capture: {
                warmupTicks: WARMUP_TICKS,
                sampleEvery: SAMPLE_EVERY,
            ...
            counts: {
                ...
                boring: entries.length - positives.length,
                stage0Interesting: positives.filter((entry) => entry.cohort === 'stage0').length,
            },
            byClass: entries.reduce((counts, entry) => {
...
async function captureReferences(appContext, priorMeta, onProgress) {
    ...
    const results = await Promise.all(REFERENCE_CASES.map(async (entry, worldIndex) => {
        ...
        return normalizeMetrics(await proxy.runEvaluation({
            ticks: entry.ticks,
            sampleEvery: SAMPLE_EVERY,
            warmupTicks: WARMUP_TICKS,
            probe: { enabled: true, probeTicks: PROBE_TICKS },
    ...
    REFERENCE_CASES.forEach((entry, index) => { output[entry.id] = results[index]; });
    return output;
}
...
/**
 * Mount the headless/dev-only deterministic statistical benchmark runner.
...
export function mountInterestingnessBenchmarkCapture(appContext, library) {
    ...
    const benchmarkButton = host.querySelector('#benchmark-run');
    ...
    referenceButton.addEventListener('click', async () => {
        setBusy(true);
        try {
            const prior = await fetch(`${import.meta.env.BASE_URL}tests/fixtures/exploreEvalFixtures.json`)
                .then((response) => response.json());
            const references = await captureReferences(appContext, prior._meta, (message) => {
                status.textContent = message;
            });
            publish(references, 'exploreEvalFixtures.json');
    ...
    benchmarkButton.addEventListener('click', async () => {
        setBusy(true);
        try {
            const prior = await fetch(`${import.meta.env.BASE_URL}tests/fixtures/interestingnessBenchmark.json`)
                .then((response) => response.json());
            const negatives = prior.entries.filter((entry) => entry.label === 'boring').map((entry) => ({
                id: entry.id,
                label: entry.label,
                source: entry.source,
                note: entry.note,
            ...
            publish(benchmark, 'interestingnessBenchmark.json');
