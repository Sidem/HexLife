...
| `@hexlife/embed/stochastic` | Wasm | Stateful/probabilistic neighborhood worlds and the conserved lattice gas, with compiled native rules, age epochs, counter RNG, census and checksums. A separately loaded artifact with no DOM at module scope. |
...
lifecycle policies as `<hexlife-ca>`: offscreen pause, hidden-tab pause, reduced motion, a DPR cap,
and context-loss recovery that rebuilds only the renderer.
