...
describe('BehaviorArchive native descriptor cells', () => {
    it('cellKeyOverride banks look-different-but-stats-alike finds in DISTINCT cells', () => {
        ...
        a.tryInsert({ hex: 'aa', score: 0.5, metrics: m, descriptorKind: 'native' }, { cellKeyOverride: 'n:1010' });
        a.tryInsert({ hex: 'bb', score: 0.6, metrics: m, descriptorKind: 'native' }, { cellKeyOverride: 'n:0101' });
        // Without the override these two would collide onto one statistical cell; the perceptual keys split them.
        expect(a.size).toBe(2);
        expect(a.getEntries().map(e => e.cellKey).sort()).toEqual(['n:0101', 'n:1010']);
    });

    it('stamps the override cellKey and keeps best-per-cell within an override cell', () => {
        const a = new BehaviorArchive();
        const m = metrics(0.3, 0.4, 1.0);
        a.tryInsert({ hex: 'lo', score: 0.4, metrics: m, descriptorKind: 'native' }, { cellKeyOverride: 'n:1111' });
        const better = a.tryInsert({ hex: 'hi', score: 0.9, metrics: metrics(0.9, 0.9, 0.3), descriptorKind: 'native' }, { cellKeyOverride: 'n:1111' });
        expect(better.improved).toBe(true);
        expect(a.size).toBe(1);
        expect(a.getEntries()[0].hex).toBe('hi');
        expect(a.getEntries()[0].cellKey).toBe('n:1111');
    });

    it('noveltyMultiplier honours the perceptual cell override', () => {
        const a = new BehaviorArchive();
        const m = metrics(0.3, 0.4, 1.0);
        a.tryInsert({ hex: 'occupy', score: 0.7, metrics: m, descriptorKind: 'native' }, { cellKeyOverride: 'n:1010' });
        // Same perceptual cell, weaker score ⇒ penalized; a DIFFERENT perceptual cell ⇒ novel (1).
        expect(a.noveltyMultiplier(m, 0.5, 'x', 'n:1010')).toBe(ARCHIVE_CONFIG.occupiedNoveltyMultiplier);
        expect(a.noveltyMultiplier(m, 0.5, 'x', 'n:0101')).toBe(1);
    });
    ...
    it('round-trips native-keyed entries: the opaque cellKey is preserved on reload', () => {
        const a = new BehaviorArchive();
        a.loadEntries([
            { hex: 'aa', score: 0.5, cellKey: 'n:1010', descriptorKind: 'native', metrics: metrics(0.3, 0.4, 1.0) },
            { hex: 'bb', score: 0.6, cellKey: 'n:0101', descriptorKind: 'native', metrics: metrics(0.3, 0.4, 1.0) },
        ...
        expect(byHex.aa).toBe('n:1010'); // preserved verbatim (not recomputed from stats)
        expect(byHex.bb).toBe('n:0101');
        expect(byHex.cc).toBe('3|5|b2'); // stats entry re-derived
    });

    it('reload keeps the best per native cell (self-heals a duplicated dump)', () => {
        const a = new BehaviorArchive();
        a.loadEntries([
            { hex: 'lo', score: 0.4, cellKey: 'n:1111', descriptorKind: 'native', metrics: metrics(0.3, 0.4, 1.0) },
            { hex: 'hi', score: 0.8, cellKey: 'n:1111', descriptorKind: 'native', metrics: metrics(0.3, 0.4, 1.0) },
        ]);
        expect(a.size).toBe(1);
        expect(a.getEntries()[0].hex).toBe('hi');
    });

    it('downgrades legacy embedding-keyed entries to portable statistical cells', () => {
        const a = new BehaviorArchive();
        a.loadEntries([{
            hex: 'legacy',
            score: 0.5,
