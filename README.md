...
#### How a candidate is scored — the eight component values

The gallery's per-component bars expose the raw ingredients of the interestingness score. Each is normalized to **[0, 1]**, then combined with the weights shown (criticality is dropped and the rest renormalized if no damage-probe ran; the two spatial terms, the temporal-variance term and the transport term are likewise dropped for legacy gallery entries that predate them):
